...
public class PlaywrightForgetMeTest : PageTest
{
    ...
    public async Task ForgetMeTest()
    {
        ...
        await Page.GetByLabel("Password", new() { Exact = true })
            .FillAsync("TestOfForgetMe@test.dk1");
        ...
        await Page.Locator("#CheepText")
            .FillAsync("Test that this is deleted when i press the 'Forget Me!' button");
        await Page.Locator("#CheepText").PressAsync("Enter");

        // Forget me
        await Page.GetByRole(AriaRole.Link, new() { Name = "About me" }).ClickAsync();
        await Expect(Page.GetByRole(AriaRole.Heading, new() { Name = "About Me" }))
            .ToBeVisibleAsync(); // Assert we are on about me page
        ...
        await Expect(Page.Locator("#messagelist"))
            .Not.ToContainTextAsync(
                "Test that this is deleted when i press the 'Forget Me!' button"
            );
        ...
        await Expect(Page.GetByRole(AriaRole.Emphasis))
            .ToContainTextAsync("There are no cheeps so far.");
