...
public class PlaywrightSecurityTest : PageTest
{
    ...
    public async Task CheepboxCanSustainXSSandSQLAttacks()
    {
        ...
        await Page.Locator("#CheepText")
            ...
            ); //input text is directly from slides from lecture 10
        await Page.GetByRole(AriaRole.Button, new() { Name = "Share" }).ClickAsync();
        await Expect(
                Page.GetByText(
                    "Robert'); DROP TABLE Cheeps;-- Hello, I am  feeling good!<script>alert('If you see this in a popup, you are in trouble!');</script>"
                ).First
            )
            .ToBeVisibleAsync();
