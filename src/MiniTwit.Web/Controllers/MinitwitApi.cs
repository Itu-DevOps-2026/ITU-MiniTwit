...
namespace Org.OpenAPITools.Controllers
{
    ...
    public class MinitwitApiController : Controller
    {
        ...
        public virtual IActionResult GetLatestValue()
        {
            ...
            {
                _logger.LogError(e, "An error occurred while getting the latest value");
                return StatusCode(
                    500,
                    new ErrorResponse { Status = 500, ErrorMsg = "Internal Server Error" }
                );
            }
        }
        ...
        )
        {
            ...
            var messages = cheeps
                ...
                .Select(c => new Message
                {
                    Content = c.Text,
                    User = c.AuthorName,
                    PubDate = c.CreatedAt.ToString("yyyy-MM-dd HH:mm:ss"),
                })
                .ToList()
                .ToJson();
        ...
        )
        {
            _logger.LogInformation(
                "called get messages per user with username {username} and amount {amount}",
                username,
                no ?? 100
            );
            _latestService.SetLatest(latest);
        ...
        {
            _logger.LogInformation(
                "called fllws post as {user} who wants to {fllws}",
                username,
                payload
            );
            _latestService.SetLatest(latest);
            ...
            )
            {
                return BadRequest(
                    ...
                    {
                        Status = 400,
                        ErrorMsg = "Exactly one of 'follow' or 'unfollow' must be provided",
                    }
                );
            }
        ...
        {
            _logger.LogInformation(
                "called msgs post with {user} and content {content}",
                username,
                payload.Content
            );
            _latestService.SetLatest(latest);
        ...
        )
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation(
                    "added author {}",
                    _authorService.GetAuthorByName(payload.Username).Result
                );
                return StatusCode(204);
            }
            ...
            return StatusCode(
                ...
                new ErrorResponse
                {
                    Status = 400,
                    ErrorMsg =
                        "Bad Request | Possible reasons:  - missing username  - invalid email  - password missing  - username already taken",
                }
            );
        }
