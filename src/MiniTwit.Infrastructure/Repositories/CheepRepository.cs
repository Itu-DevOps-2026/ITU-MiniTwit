...
public class CheepRepository : ICheepRepository
{
    ...
    public async Task<List<CheepDTO>> ReadCheepsBySelfAndOthers(
        IList<string> authorNames,
        int? page = null
    )
    {
        // Construction of the query that selects cheeps written by the authorName
    ...
    private async Task UpdateCheep(Cheep originalCheep, CheepDTO alteredCheep)
    {
        ...
        {
            throw new Exception(
                $"Author with username '{alteredCheep.AuthorId}' not found while trying to update cheep."
            );
        }
    ...
    private async Task<Author?> FindAuthor(string authorId)
    {
        ...
        var query = from author in _context.Authors where author.Id == authorId select author;
